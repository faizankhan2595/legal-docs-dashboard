...
const NewOrder = (props) => {
  ...
  const submitDocs = () => {
    ...
    if (formURLs.length === 0) {
      message.error("Please upload filled form");
      return false;
    }
    ...
    if (formURLs.length > 1) {
      message.error(
        `You can only upload one form, please delete other files under "Upload Filled Form" section`
      );
      return false;
    }
  ...
  return (
    <>
      ...
      >
        Download order form from this{" "}
        <a
          href="https://apostille.inkapps.io/assets/Order Form.pdf"
          target="_blank"
          rel="noreferrer"
        >
          link
